...
class ShuffleNet(nn.Module):
    def __init__(self, heads, head_conv, n_class=1000, input_size=224, width_mult=1.):
        ...
        self.inplanes = 24  # yy add
        self.deconv_with_bias = False # yy
        assert input_size % 32 == 0

        self.stage_repeats = [4, 8, 4]
        # if  #self.stage_repeats = [2, 3, 2] ?
...
def shufflenetv2(width_mult=0.5):  # 1.
    model = ShuffleNet(width_mult=width_mult)
    return model
...
def get_shufflev2_net(num_layers, heads, head_conv):
  model = ShuffleNet(heads, head_conv, width_mult=0.5)
  return model
