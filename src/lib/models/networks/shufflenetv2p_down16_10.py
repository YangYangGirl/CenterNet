...
class CEM3(nn.Module):
    def __init__(self, channel, reduction=16):
        super(CEM3, self).__init__()
    ...
    def forward(self, x):
        ...
        # print("after pool", y.shape)
        # print("after fc", self.fc(y).shape)
...
class PIM_up(nn.Module):
    def __init__(self, inchannel, outchannel):
        super(PIM_up, self).__init__()
        self.net = nn.Sequential(nn.Conv2d(inchannel, outchannel, 3, 1, 0, bias=False),
...
class ShuffleNetV2(nn.Module):
    def __init__(self, heads, head_conv, n_class=1000, input_size=224, width_mult=1.):
        ...
        if width_mult == 0.5:
            self.stage_out_channels = [-1, 24, 48, 96, 192, 256]
        elif width_mult == 1.0:
        ...
        self.inplanes = 144  # self.stage_out_channels[-2] #256

        oup_inc = self.stage_out_channels[2]
        self.cem1 = nn.Sequential(
            # SELayer(self.stage_out_channels[4]),
            # pw
            nn.Conv2d(self.stage_out_channels[4], self.stage_out_channels[2], 1, 1, 0, bias=True),
            nn.BatchNorm2d(self.stage_out_channels[2]),
            # dw
            nn.Conv2d(oup_inc, oup_inc, 5, 1, 1, groups=oup_inc, bias=True),
            nn.BatchNorm2d(oup_inc),
            # pw
            nn.Conv2d(self.stage_out_channels[2], self.stage_out_channels[2], 1, 1, 1, bias=True),
            nn.BatchNorm2d(self.stage_out_channels[2]),
            # nn.ReLU(inplace=True)
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
        )
        ...
        self.cem3 = nn.Sequential(
            # pw
            nn.Conv2d(self.stage_out_channels[3], self.stage_out_channels[2], 1, 1, 0, bias=True),
            nn.BatchNorm2d(self.stage_out_channels[2]),
            # dw
            nn.Conv2d(oup_inc, oup_inc, 5, 1, 1, groups=oup_inc, bias=True),
            nn.BatchNorm2d(oup_inc),
        ...
        #         self.cem1 = nn.Sequential(
        #                    SELayer(self.stage_out_channels[2]),
        #                    nn.Conv2d(self.stage_out_channels[2], self.stage_out_channels[4], 1, 2, 0, bias=False),
        #                    nn.BatchNorm2d(self.stage_out_channels[4]),
        #                    nn.ReLU(inplace=True)
        #                    )
        #         self.cem2 = nn.Sequential(
        #                     #SELayer(232),
        #                     conv_1x1_bn(self.stage_out_channels[3], self.stage_out_channels[4]), #prepare to cat
        #                     nn.BatchNorm2d(self.stage_out_channels[4]),
        #                     nn.ReLU(inplace=True)
        #                     )
        ...
        self.PIM = PIM_up(48, 48)

        self.cemse = SELayer(self.stage_out_channels[4])
        # add heads
        self.heads = heads
    ...
    def forward(self, x):
        ...
        # print(cem1_input.shape)
        # print(cem2_input.shape)
        # print(cem3_input.shape)
        #
        # print(cem1_output.shape)
        # print(cem2_output.shape)
        # print(cem3_output.shape)

        x = torch.cat([cem1_output, cem2_output, cem3_output], 1)
...
def shufflenetv2(width_mult=0.5):
