    ...
    def __getitem__(self, index):
      ...
      for k in range(num_objs):
        ...
        if h > 0 and w > 0:
          ...
          lk = np.array(
            [bbox[0], bbox[1], ct[0], ct[1]], dtype=np.float32)
          lk_int = lk.astype(np.int32)
          reg_lk = lk - lk_int
          ...
          reg[k] = ct - ct_int
          reg_mask[k] = 1
          cat_spec_wh[k, cls_id * 2: cls_id * 2 + 2] = wh[k]  #根据类别的wh
      ...
      #ind 为中心点的整数坐标； reg为中心点的offset
      ret = {'input': inp, 'lk': lk, 'reg_lk': reg_lk, 'reg_mask': reg_mask}
      if self.opt.dense_wh:
